...
class EnviaProcessor {
    ...
    async validateXML(xmlString, xsdPath) {
        var _a;
        ...
        try {
            ...
            return {
                isValid: false,
                xmlErrors: (_a = errorMessage === null || errorMessage === void 0 ? void 0 : errorMessage.split("\n")) === null || _a === void 0 ? void 0 : _a.filter((line) => line.trim() !== ""),
            };
        }
        finally {
    ...
    async transmitirXml(xmlLote, nfeObj) {
        ...
        try {
            ...
            if (retornoEnvio && retornoEnvio.data) {
                ...
                throw retornoEnvio.error || new Error("Erro ao realizar requisição");
            }
    ...
    gerarNFe(documento, dadosChave) {
        ...
        nfe.infIntermed = this.getInfoIntermed(documento.infoIntermediador);
    ...
    getIde(documento, dadosChave) {
        ...
        let ide = {
            ...
            indIntermed: documento.indIntermed
                ? Utils.getEnumByValue(schema.TNFeInfNFeIdeIndIntermed, documento.indIntermed)
                : undefined,
        ...
        if ((_a = documento.nFref) === null || _a === void 0 ? void 0 : _a.length) {
            ide.NFref = documento.nFref.map((ref) => this.getNFref(ref));
        }
        return ide;
    }
    getNFref(ref) {
        const nFref = {};
        if (ref.refNFe)
            nFref.refNFe = ref.refNFe;
        if (ref.refNFeSig)
            nFref.refNFeSig = ref.refNFeSig;
        if (ref.refCTe)
            nFref.refCTe = ref.refCTe;
        if (ref.refNF) {
            nFref.refNF = {
                cUF: ref.refNF.cUF,
                AAMM: ref.refNF.AAMM,
        ...
        for (let i = 0; i < produtos.length; i++) {
            det_list.push({
                ...
                impostoDevol: produtos[i].prod.percentualDevolucao &&
                    produtos[i].prod.valorIPIDevolucao &&
                    Number(produtos[i].prod.percentualDevolucao) > 0
                    ? this.getImpostoDevolucao({
                        pDevol: Number(produtos[i].prod.percentualDevolucao),
                        vIPIDevol: Number(produtos[i].prod.valorIPIDevolucao),
                    })
                    : undefined,
            });
        }
    ...
    getImpostoIPI(ipi, modelo) {
        ...
        if (Number(ipi.vBC || 0) + Number(ipi.pIPI || 0) > 0 &&
            Number(ipi.qUnid || 0) + Number(ipi.vUnid || 0) > 0)
        ...
        if (!["00", "49", "50", "99"].includes(ipi.CST)) {
            result.IPINT = {
                CST: Utils.getEnumByValue(schema.TIpiIPINTCST, ipi.CST),
            };
            return result;
        }
        if (Number(ipi.qUnid || 0) + Number(ipi.vUnid || 0) > 0) {
            result.IPITrib = {
                CST: ipi.CST,
                qUnid: ipi.qUnid,
                vUnid: ipi.vUnid,
    ...
    getImpostoPIS(pis, modelo) {
        ...
        switch (pis.CST) {
            ...
            case "98":
            case "99":
                if (Number(pis.vBC || 0) + Number(pis.pPIS || 0) > 0 &&
                    Number(pis.qBCProd || 0) + Number(pis.vAliqProd || 0) > 0)
    ...
    getImpostoCOFINS(cofins, modelo) {
        ...
        switch (cofins.CST) {
            ...
            case "98":
            case "99":
                if (Number(cofins.vBC || 0) + Number(cofins.pCOFINS || 0) > 0 &&
                    Number(cofins.qBCProd || 0) + Number(cofins.vAliqProd || 0) > 0)
    ...
    getVolume(vol) {
        const result = {};
        if (vol.qVol != null && vol.qVol !== "")
            result.qVol = String(vol.qVol);
        if (vol.esp)
            result.esp = vol.esp;
        if (vol.marca)
            result.marca = vol.marca;
        if (vol.nVol)
            result.nVol = vol.nVol;
        if (vol.pesoL != null && vol.pesoL !== "")
            result.pesoL = String(vol.pesoL);
        if (vol.pesoB != null && vol.pesoB !== "")
            result.pesoB = String(vol.pesoB);
        if (vol.lacres && vol.lacres.length > 0) {
            const lacres = vol.lacres
                .filter((l) => l && l.nLacre)
                .map((l) => ({ nLacre: l.nLacre }));
            if (lacres.length > 0)
                result.lacres = lacres;
        }
        return result;
    }
    getCobr(cobranca) {
    ...
    getInfoIntermed(info) {
        if (!info)
            return undefined;
        return {
            CNPJ: info.CNPJ,
            idCadIntTran: info.idCadIntTran,
        };
    }
